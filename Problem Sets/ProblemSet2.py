...
"""
Problem 2_1:
Write a function 'problem2_1()' that sets a variable lis = list(range(20,30)) and does all of the following, each on a separate line: 
  (a) print the element of lis with the index 3
  (b) print lis itself
  (c) write a 'for' loop that prints out every element of list. Recall that len() will give you the length of such a data collection if you need that. Use end=" " to put one space between the elements of the list lis.
  >>>problem2_1()
  23
...
"""
Problem 2_2:
Write a function 'problem2_2()' that creates a list called alist. (Actually, I've started the function for you below. Your function should do all of the following, each on a separate line. Recall that lists start numbering with 0.0) print the whole list (this doesn't require a while or for loop)
  ...
  Make sure that your function also works with blist below.  For this to work, you cannot use a list as a variable inside your function.
  >>>problem2_2(alist)
  ['a', 'e', 'i', 'o', 'u', 'y']
  a
  y
...
"""
Problem 2_3:
Write a function problem2_3() that should have a 'for' loop that steps through the list below and prints the name of the state and the number of
...
"""
Problem 2_4:
random.random() generates pseudo-random real numbers between 0 and 1. But what if you needed other random reals? Write a program to use only random.random() but generates a list of random reals between 30 and 35. This is a simple matter of multiplication and addition. Print out the list (in list form).

https://d18ky98rnyall9.cloudfront.net/_e084396f9c477a9c5f5ad8b4cdc2119d_Exercises2.py?Expires=1480636800&Signature=dYnMKknQncESvCluShZcnXzB5-nNfh33WJ3l26FF-mNL2wPYFjv7pY3SdNDUeRMQVchVlNFFDx1eXVONanOuofxX9-a-zbZ4PmiDwT72hELsMt4vGB2StNS5U-n3D2kPSavlIxLK9E5FPZTeTQyFrUUQEFFanw8-KPDvHiduK-I_&Key-Pair-Id=APKAJLTNE6QMUY6HBC5A

COMMENT: Note that this uses a pseudorandom number generator.  That means that the list will be different for each person.  But all elements of the list need to be between 30 and 35 inclusive.  How do we do that with the Coursera grader? 
One way to handle this is to issue the command random.seed(70) inside problem2_4(). Then the list generated will that one given below --- always. You should use random.random() to generate a random number between 0 and 1 and then use arithmetic to convert that to a random number in the required range.
...
def problem2_4():
    ...
    random.seed(70)
    lst = [(random.random() * 5) + 30 for i in range(0,10)]
    print(lst)

#%%
"""
Problem 2_5:
Let's do a small simulation. Suppose that you rolled a die repeatedly. Each time that you roll the die you get a integer from 1 to 6, the number of pips on the die. Use random.randint(a,b) to simulate rolling a die 10 times and printout the 10 outcomes. The function random.randint(a,b) will generate an integer (whole number) between the integers a and b inclusive. Remember each outcome is 1, 2, 3, 4, 5, or 6, so make sure that you can get all of these outcomes and none other. Print the list. Make sure that it has 10 items and they are all in the range 1 through 6.  Here is one of my runs. In the problem below I ask you to set the seed to 171 for the benefit of the auto-grader. In this example, that wasn't done and so your numbers will be different.
...
"""
Problem 2_6:
Let's continue with our simulation of dice by rolling two of them. This time each die can come up with a number from 1 to 6, but you have two of them. The result or outcome is taken to be the sum of the pips on the two dice. Write a program that will roll 2 dice and produce the outcome. This time let's roll the two dice 100 times. Print the outcomes one outcome per line.
...
#%%
"""
Sample run using the list hourly_temp. Note that the grader will use a different hourly list.  Be sure that you function works on this list and test it on at least one other list of your own construction.
